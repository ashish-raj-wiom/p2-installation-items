...
5. **The new team member is notified using whatever notification path that family already uses for the initial assignment.** Specifically: **for Install**, that path is the FPN treatment defined in [[Item 04]] — explicitly *"every assignment including re-assignments"* per Item 04's PM-confirmed scope (hard dependency below). **For Restore and NBREC**, the notification path is whatever they already fire today on first-assignment — this item just says the same notification extends to a swap. Item 04 does *not* cover Restore / NBREC; if PM wants those families to also have FPN treatment, that's a separate item.
...
1. **CSP changes the team member after the assigned technician has already arrived at site (`ARRIVED_AT_SITE` for Install, `IN_PROGRESS` for Restore).** Allowed per PM — mid-work swap permitted. The new team member is notified using the family's existing initial-assignment notification path (for Install, this is the FPN treatment from [[Item 04]] which explicitly covers re-assignments; for Restore / NBREC, the notification used on first-assign extends to the swap). Previously-on-site tech's app removes the booking from their active list on next refresh (the "stale-card-clears" pattern Item 04 documents for the Install / Technician app — same behaviour applies to Restore / NBREC by their respective UX rules). Practical caveat: any session state the previous tech accumulated (selfie taken, OTP entered, photos captured) needs to be either carried over or restarted — tech-design decision per family.
2. **CSP changes the team member while a customer call is in progress** (per [[IVR-Routing-Solutioning]] dual-PIN model). The IVR session is bound to the original technician's PIN. New tech is notified via the family's assignment path (FPN for Install per Item 04; per-family notification elsewhere); if they need to call the customer, they use their own PIN, which IVR routes to the new tech. Existing call doesn't migrate; this is consistent with the IVR design.
...
4. **CSP rapidly swaps team members multiple times in a row** (e.g. picks A, picks B 5 seconds later, picks C). Each swap fires a fresh assignment notification to the new pick (FPN for Install per Item 04; per-family notification elsewhere) and increments the reassignment counter (if tracked). No rate limit — partner UI's responsibility to debounce if needed.
...
- **Hard dependency on Item 04 — Install only.** For the **Install** family, this item assumes the FPN-on-assignment treatment already extends to re-assignments. Item 04 explicitly captures *"every assignment incl. re-assignments"* in its Install scope — so Item 04 must ship before (or alongside) this item for the install swap to fire an FPN to the new tech. **Restore and NBREC are outside Item 04's scope** — they each use their own existing assignment-notification path, which this item just extends to fire on swap too. If PM later wants FPN treatment on Restore/NBREC swaps as well, file a separate item analogous to Item 04 for those families.
...
- **Related items:** [[Item 04]] (Install-only technician-assignment FPN — hard dependency for the FPN-on-swap behaviour in the Install family; does **not** extend to Restore / NBREC, which use their own per-family assignment notification paths) · [[IVR-Routing-Solutioning]] (call-routing implication during a swap, edge case 2).
